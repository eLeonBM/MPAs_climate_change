...
bar<- mpas_cvi %>%
        ...
        lims(y=c(0,11))+
        # geom_bar(width =1 )+
        scale_fill_manual(values = c("#005a32", "#ede61a", "#ed671a", "firebrick"), name = "Category") +
        theme_ipsum() +
        ...
        theme(strip.text.x = element_text(angle = 0, hjust = .5),
              ...
              axis.title.y=element_text(face="bold", hjust =0.5, family="Times New Roman", size=18),
...
bar
ggsave("Graphics/MPA_CVI_bars.png", dpi=500, height= 8, width= 12)
...
spdf_mx <-  st_transform(st_as_sf(rnaturalearth::ne_countries(country = 'mexico')), crs = 4326)#Selecting Mexico

## LOW
p1 <- ggplot() + geom_sf(data=spdf_mx,
                               fill= "gray90",
                               col= NA)+
        ggplot2::geom_sf(data = low, fill = "#005a32") +
        geom_label_repel(data = low, aes(x = lon, y = lat, label = ID), #function to plot label ID to polygons
        ...
        coord_sf(xlim = c(-118, -85), ylim =c(13,32), expand= TRUE)+
        ...
        theme(plot.title = element_text(size=20, face="bold",family= "Times New Roman", hjust=0.5 ),
              plot.subtitle = element_text(size=18, family= "Times New Roman", hjust=0.5),
              legend.position = "")+
        ggspatial::annotation_scale()
p1

ggsave("Graphics/MPA_CVI_Low.png", dpi=700, height= 8, width= 12)  

## MEDIUM
p2 <- ggplot() + geom_sf(data=spdf_mx,
                   fill= "gray90",
                   col= NA)+
        ggplot2::geom_sf(data = medium, fill = "#ede61a") +
        geom_label_repel(data = medium, aes(x = lon, y = lat, label = ID),
                         box.padding = unit(0.5, "lines"),
                         # point.padding = unit(0.5, "lines"),
                         segment.color = 'black',
                         label.padding = 0.1,
...
p2
ggsave("Graphics/MPA_CVI_Medium.png", dpi=700, height= 8, width= 12) 


## HIGH
p3 <- ggplot() + geom_sf(data=spdf_mx,
                         fill= "gray90",
                         col= NA)+
        ggplot2::geom_sf(data = high, fill = "#ed671a") +
        geom_label_repel(data = high, aes(x = lon, y = lat, label = ID),
                         box.padding = unit(0.5, "lines"),
                         # point.padding = unit(0.5, "lines"),
                         segment.color = 'black',
                         label.padding = 0.1,
        ...
        ggspatial::annotation_scale()
p3
ggsave("Graphics/MPA_CVI_High.png", dpi=700, height= 8, width= 12)  


## VERY HIGH
p4 <- ggplot() + geom_sf(data=spdf_mx,
                         fill= "gray90",
                         col= NA)+
        ggplot2::geom_sf(data = very_high, fill = "firebrick") +
        geom_label_repel(data = very_high, aes(x = lon, y = lat, label = ID),
                         box.padding = unit(0.5, "lines"),
                         # point.padding = unit(0.5, "lines"),
                         segment.color = 'black',
                         label.padding = 0.1,
        ...
        ggspatial::annotation_scale()
p4
ggsave("Graphics/MPA_CVI_Very_High.png", dpi=700, height= 8, width= 12)  
